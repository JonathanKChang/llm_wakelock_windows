...
The system SHALL encapsulate all Windows TCP table retrieval in a `WindowsTcpHandler` class and all WSL2 TCP polling in a `Wsl2TcpHandler` class. The main loop SHALL call `handler.get_connections()` through the shared Protocol interface.
